...
resource "azurerm_network_security_rule" "nsgrules" {
  ...
  name                         = jsondecode(var.nsg_rules_map[each.value]).name
  priority                     = jsondecode(var.nsg_rules_map[each.value]).priority
  direction                    = jsondecode(var.nsg_rules_map[each.value]).direction
  access                       = jsondecode(var.nsg_rules_map[each.value]).access
  protocol                     = jsondecode(var.nsg_rules_map[each.value]).protocol
  source_port_ranges           = jsondecode(var.nsg_rules_map[each.value]).source_port_ranges
  destination_port_ranges      = jsondecode(var.nsg_rules_map[each.value]).destination_port_ranges
  source_address_prefixes      = sensitive(jsondecode(var.nsg_rules_map[each.value]).source_address_prefixes)
  destination_address_prefixes = sensitive(jsondecode(var.nsg_rules_map[each.value]).destination_address_prefixes)
